...
def makeGreeting():
    ...
    word_selected = random.choice(wakeUpWordsList)
    return word_selected[0].upper()+word_selected[1:]


def getPersonName(userText):
...
def addText(userText, addOn):
    ...
    final_text = chatWindow.get("1.0", "end-1c")+"\n"+addOn+userText
    chatWindow.delete(1.0,"end")
    chatWindow.insert(1.0, final_text)


def send(userText, isUser):
...
def enterPressed(event=None):
    ...
    if userText in wakeUpWordsList:
        ...
        send(compText, False)
        getVoiceResponse(compText)
    if "bye" in userText:
        getVoiceResponse("Bye")
        root.destroy()
        sys.exit()
    if "date" in userText:
        compText = getDate()
    ...
    if "who is" in userText:
        ...
        try:
            compText = wikipedia.summary(getPersonName(userText), sentences=2)
            send(compText, False)
            getVoiceResponse(compText)
        except wikipedia.exceptions.DisambiguationError:
            send("I do not know this person. Please be more precise.", False)
            getVoiceResponse("I do not know this person. Please be more precise.")
    elif compText == "":
        #"I do not understand" error checks
        send("I do not understand. Please try again.", False)
        getVoiceResponse("I do not understand. Please try again.")


def buildGUI():
    '''Build the main Chatbox GUI.
    ...
    global root, userTextField, chatWindow, wakeUpWordsList
    
    root = Tk()
    root.title("Chatbot"), root.geometry("300x300"), root.resizable(False, False)

    userTextField = StringVar(None)
    userTextEntry = Entry(root, textvariable=userTextField, width=25)
    userTextEntry.place(x=0,y=272)

    chatWindow = Text(root, bd=1, background="yellow")
    chatWindow.place(x=0,y=0,width=298,height=273)
    chatWindow.insert(1.0,"Hello I am your assistant.\nHow can I help you?")

    sendButton = Button(root, text="Send", padx=12, pady=3.4, command=enterPressed)
    sendButton.place(x=236,y=273)
    userTextEntry.bind('<Return>',enterPressed)

    wakeUpWordsList = ("hey", "hi there", "hello", "greetings", "salutations", "hi", "yo")
    root.mainloop()
